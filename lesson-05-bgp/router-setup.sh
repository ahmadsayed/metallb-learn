#!/usr/bin/env bash
# Lesson 5 — stand up the "datacenter router" that MetalLB will peer with.
#
# In production this is a switch/router (Arista, Juniper, Cumulus…) that your
# network team configures. In the lab it is an FRR container sitting on the
# same Docker network as the kind nodes, with a static address.
#
...
METALLB_ASN="${METALLB_ASN:-64512}"    # MetalLB's AS (set in the BGPPeer CR)
...
echo "=== 5/5 Peer with every cluster node ($ROUTER_ASN <- $METALLB_ASN) ==="
ARGS=(-c 'configure terminal' -c "router bgp $ROUTER_ASN" -c 'no bgp ebgp-requires-policy')
for N in "${NODES[@]}"; do ARGS+=(-c "neighbor $N remote-as $METALLB_ASN"); done
ARGS+=(-c 'end')
...
echo
echo "Router ready. Now apply metallb-bgp.yaml and check: docker exec $ROUTER vtysh -c 'show bgp summary'"
